  ...
  return (
    <ul className={` ${windowsScreen<768 ? 'absolute bg-white w-full' : ''} ${navToggle ? 'block ' :'hidden'}   md:flex md:flex-row items-center md:space-x-7    right-0 top-[80px] h-fit py-5`}  >
      <li className="list-none  md:text-white md:border-white md:border-none  border-b pl-2 py-2 md:pl-0 md:py-0 font-semibold  ">
        <NavLink onClick={handleNavLinkClick}   className={({isActive}) => `${isActive && 'active-link '} ${navbarScrolled ? ' text-black navlink-scroll ' : 'navlink'} relative ${isActive && navbarScrolled ? ' active-link-scroll':''}`} to="/">Home</NavLink>
      </li>
      <li className="list-none  md:text-white md:border-white md:border-none  border-b pl-2 py-2 md:pl-0 md:py-0 font-semibold ">
        <NavLink onClick={handleNavLinkClick}  className={({isActive}) => `${isActive && 'active-link '} ${navbarScrolled ? ' text-black navlink-scroll  ' : 'navlink'} relative ${isActive && navbarScrolled ? ' active-link-scroll':''}`} to="/about">About Us</NavLink>
      </li>
      <li className="list-none  md:text-white md:border-white md:border-none  border-b pl-2 py-2 md:pl-0 md:py-0 font-semibold ">
        <NavLink onClick={handleNavLinkClick}  className={({isActive}) => `${isActive && 'active-link '} ${navbarScrolled ? ' text-black navlink-scroll ' : 'navlink'} relative  ${isActive && navbarScrolled ? ' active-link-scroll':''}`} to="/courses">Courses</NavLink>
      </li>
      <li className="list-none  md:text-white md:border-white md:border-none  border-b pl-2 py-2 md:pl-0 md:py-0 font-semibold ">
        <NavLink onClick={handleNavLinkClick}  className={({isActive}) => `${isActive && 'active-link '} ${navbarScrolled ? ' text-black navlink-scroll ' : 'navlink'} relative ${isActive && navbarScrolled ? ' active-link-scroll':''}`} to="/contact">Contact</NavLink>
      </li>
      <Link to={'/courses'}>
